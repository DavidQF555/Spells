    ...
    public void run() {
        if (!Main.traps.isEmpty())
            for (final Block trap : Main.traps) {
                entities.clear();
                entities.addAll(trap.getWorld().getNearbyEntities(trap.getLocation(), 3, 3, 3));
                for (Entity entity : entities) {
                    ...
                    if (steppedOnBlock.equals(trap)) {
                        ...
                        Location aboveTrap = new Location(aboveTrapBlock.getWorld(), aboveTrapBlock.getX() + 0.5, aboveTrapBlock.getY(), aboveTrapBlock.getZ() + 0.5);
